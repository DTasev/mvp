...
class TofConverterModel(object):
    ...
    def doConversion(self, inputval, inOption, outOption, theta, flightpath):
        stage1output = self.input2energy(float(inputval), inOption, theta, flightpath)
        stage2output = self.energy2output(stage1output, outOption, theta, flightpath)
        return stage2output

    # Convert input value to intermediate energy

    def input2energy(self, inputval, inOption, theta, flightpath):
    ...
    def energy2output(self, Energy, outOption, theta, flightpath):
        e2lam = 81.787  # using lambda=h/p  p: momentum, h: planck's const, lambda: wavelength
        e2nu = 4.139  # using h/(m*lambda^2) m: mass of neutron
        e2v = 0.0000052276  # using v = h/(m*lambda)
        e2k = 2.0717  # using k = 2*pi/(lambda) k:momentum
